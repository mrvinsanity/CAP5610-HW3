...
def repeatedKMeans(instances, k=10, n=100):
    bestClustering = {}
...
def computeWithinss(clusters, centroids):
    ...
    for i in range(len(clusters)):
        ...
        for instance in clusters[i]:
            dist = distance(instance, centroid)
            sse += dist**2
    return sse
    
    result = kmeans(instance, k=10)
    sse = computeWithinss(result["clusters"], result["centroids"])
    print("SSE: %.2f" % sse)
    
def computeSSE(clusters, centroids):
    sse = 0
    for i in range(len(clusters)):
        centroid = centroids[i]
        cluster = clusters[i]
