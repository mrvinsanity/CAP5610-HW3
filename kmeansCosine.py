...
def distance(instance1, instance2):
    ...
    for i in range(1, len(instance1)):
        dotProduct += instance1[i] * instance2[i]
        mag1 += math.pow(instance1[i], 2)
        mag2 += math.pow(instance2[i], 2)
    mag1 = math.sqrt(mag1)
    mag2 = math.sqrt(mag2)
    if mag1 == 0 or mag2 == 0:
        return float("inf")
...
def kmeans(instances, k=10, animation=False, initCentroids=None):
    result = {}
    ...
    if animation:
        ...
        time.sleep(delay)
    iteration = 0
    sse = float("inf")  # initialize SSE to infinity
    while (centroids != prevCentroids):
        ...
        newSSE = 0  # calculate the new SSE
        for i in range(len(clusters)):
            centroid = centroids[i]
            for instance in clusters[i]:
                newSSE += distance(instance, centroid)**2
        if newSSE == sse:
            break  # exit the loop if SSE doesn't change anymore
        sse = newSSE
    ...
    result["sse"] = sse
    if animation:
        paintClusters2D(canvas, clusters, centroids, "Final result")
        time.sleep(delay)
    return result
...
def repeatedKMeans(instances, k, n=100):
    bestClustering = {}
...
dataset = loadCSV("/home/kali/Documents/kmeans_data/data.csv")
